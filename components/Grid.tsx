...
const SkeletonOne: React.FC<SkeletonProps> = () => {
  ...
  return (
    ...
    >
      <motion.div
        variants={variants}
        className="flex flex-row rounded-full border border-white/[0.2] p-2 items-center space-x-2 bg-black/10 backdrop-blur-sm"
      >
        <div className="h-6 w-6 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 shrink-0" />
        <div className="w-full bg-white/20 h-4 rounded-full" />
      </motion.div>
      <motion.div
        variants={variantsSecond}
        className="flex flex-row rounded-full border border-white/[0.2] p-2 items-center space-x-2 w-3/4 ml-auto bg-black/10 backdrop-blur-sm"
      >
        <div className="w-full bg-white/20 h-4 rounded-full" />
        <div className="h-6 w-6 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 shrink-0" />
      </motion.div>
    ...
    >
      {widths.map((width, i) => (
        <motion.div
          ...
          className="flex flex-row rounded-full border border-white/[0.2] p-2 items-center space-x-2 bg-black/10 backdrop-blur-sm w-full h-4"
        />
      ))}
    </motion.div>
  );
...
const GridItem: React.FC<GridItemProps> = ({ item }) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative flex flex-col justify-between overflow-hidden rounded-xl border border-white/10 bg-black/5 p-4 sm:p-6 backdrop-blur-sm ${item.className}`}
    >
      <div className="absolute top-2 sm:top-4 right-2 sm:right-4">
        <Icon className="h-4 w-4 sm:h-6 sm:w-6 text-white/80" />
      </div>
...
const Grid: React.FC = () => {
  return (
    <section id="about" className="py-8 sm:py-12 md:py-16 bg-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 auto-rows-auto">
